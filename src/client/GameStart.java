...
public class GameStart extends JFrame {
	...
	private JTextArea taUserList2; // 유저 목록 라벨
	...
	private JButton btnExit2; // 나가기 버튼
	...
	class MyPanel extends JPanel {
		private ImageIcon icon = new ImageIcon("img/catchMindBG.png"); // 맨 처음 화면 이미지
...
//휴지통 이미지
	class MyButton1 extends JButton {
		private ImageIcon icon = new ImageIcon("img/70245.png"); // 휴지통이미지 바꿔보기
		private Image imgMain = icon.getImage();
	...
	private void init() {
		...
		btnSkip = new JButton("Pass"); // plTop
		btnReady = new JButton("Ready"); // plEast
		btnExit = new JButton("Exit"); // plEast

		btnBlackDrawPen = new JButton(iconBlackPen); // Jbutton: Creates a button with text.
		...
		// Used to set the horizontal scroll bar policy so that horizontal scrollbars
	...
	private void setting() {
		setTitle("캐치마인드");
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);// 윈도우 창을 종료시 프로세스까지 깔금하게 닫음으로써 메모리 관리가능
		setResizable(false);
